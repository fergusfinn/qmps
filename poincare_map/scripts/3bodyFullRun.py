...
def get_map(results,t):
    ...
    for x in x_points:
        ...
        θ1s.append(θ1_0)
        θ3s.append(θ3_0)
    th3 = np.mod(np.array(θ3s),2*np.pi)/np.pi
    th1 = np.mod(np.array(θ1s),np.pi * 2)/np.pi
    
    upper_half_th3= th3#[th3>1]
    upper_half_th1 = th1#[th3>1]
    # if len(upper_half_th3) >6:
    #     order = [0,7,4,1,5,2,6,3,0]
    # else:
    #     order = [0,4,1,5,2,3,0]
    #print(upper_half_th3,"\n",upper_half_th1)
    plt.plot(upper_half_th3, upper_half_th1, '.', ms = 1)
...
def viridis(r):
    cm = plt.cm.viridis
    plt.gca().set_prop_cycle(plt.cycler('color', cm(np.linspace(0, 1, r))))
...
if __name__ == "__main__":  
    ...
    viridis(18)
    # first_angle = np.loadtxt("80000step_0001.txt")
    # get_map(first_angle, range(120000))
    #starting_angles = []
    
      
    for i in range(3):
        f = f"./results/{i}_100k_0002.txt"
        angles = np.loadtxt(f)
        get_map(angles, range(100000))
    
    for i in range(12):
        f = f"./results/{i}_100k_0002_second_round.txt"
        angles = np.loadtxt(f)
        # plot_angles(angles)
        #starting_angles.append(list(angles[-1,:]))
        get_map(angles, range(100000))
        
    # for i in [0,1,3]:
    #     f = f"./results/{i}_120k_0001.txt"
    #     angles = np.loadtxt(f)
    #     #plot_angles(angles)
    #     #starting_angles.append(list(angles[-1,:]))
    #     get_map(angles, range(120000))

    #plt.savefig("../images/sim_3_body.png")
   # for i in range(9):
    #     plt.figure()
    #     plt.title(f"{i}")
    #     f = f"./results/{i}_120k_0001.txt"
...
#################################################################################
    # steps = 100000
    # dt = 0.002
    # linspace1 = np.linspace(5.8,6.2,3)
    # linspace2 = np.linspace(8.8,9.2,3)
    # x,y = np.meshgrid(linspace1, linspace2)
    # for i in range(len(x)):
    #     for j in range(len(y)):
    #         starting_angles.append([x[i,j], 0, y[i,j]])
    
    
    # quantum_angles = Parallel(n_jobs=-1)(delayed(simulate_scars)(i,[dt,steps]) for i in tq.tqdm(starting_angles))
    
    # for num, res in enumerate(quantum_angles):
    #     np.savetxt(f"./results/{num}_100k_0002_second_round .txt", res) 
    ...
    #angles_longer_step = np.loadtxt("./80000step_0001.txt")
    #angles_short_step = np.loadtxt("./160000step_00005.txt")
    
    # plot_angles(angles_short_step)
    #get_map(angles_short_step, range(160000))
