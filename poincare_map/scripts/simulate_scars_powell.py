...
def simulate_scars(initial_params, params):
    dt, timesteps = params
    H = lambda mu:0.5*(multi_tensor([I,P,X,P]) + multi_tensor([P,X,P,I])) + (mu/4) * (multi_tensor([I,I,I,n]) + multi_tensor([I,I,n,I]) + multi_tensor([I,n,I,I]) + multi_tensor([n,I,I,I]))
    ...
    for _ in range(timesteps):
        ...
        res = minimize(scars_time_evolve_cost_function, current_params, args = (current_params, hamiltonian), options = {'disp':False,'xtol':1e-10, 'ftol':1e-10}, method = 'Powell')
...
if __name__ == "__main__":
    ...
    steps = 1200
    dt = 0.1
    init_conds = [0.9, float(arguments.th1), float(arguments.th2), float(arguments.th3)]
    params = [dt, steps]
    angles = simulate_scars(init_conds, params)
    data = {'data':angles}
    filename = "/home/ucapjmd/Scratch/output/results_Powell_" + arguments.id + ".yml"
